...
REPO_MANIFESTS_URL ?= https://bitbucket.nextgen.com/scm/dso/common-automation-framework.git
...
REPO_URL ?= https://bitbucket.nextgen.com/scm/dso/git-repo.git
REPO_REV ?= main
export REPO_REV REPO_URL

# Example variable to substituted after init, but before sync in repo manifests.
GITBASE ?= https://bitbucket.nextgen.com/scm/dso/
GITREV ?= main
